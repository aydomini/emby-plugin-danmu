    ...
    public class LibraryManagerEventsHelper : IDisposable
    {
                    ...
                    foreach (var scraper in scrapers)
                    {
                        try
                        {
                            ...
                            foreach (var (episode, idx) in episodes.AsEnumerable().Reverse().WithIndex())
                            {
                                ...
                                // 使用 indexNumber - 1 作为索引，而非遍历索引 idx
        ...
        private async Task SaveDanmu(AbstractScraper scraper, BaseItem item, byte[] bytes)
        {
            ...
            if (this.Config.ToAss && bytes.Length > 0)
            {
                ...
                Danmaku2Ass.Bilibili.GetInstance().SetEmojiFilter(this.Config.AssRemoveEmoji).Create(bytes, assConfig, assPath);
            }
        }
