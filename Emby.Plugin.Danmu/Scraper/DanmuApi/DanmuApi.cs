...
namespace Emby.Plugin.Danmu.Scraper.DanmuApi
{
    public class DanmuApi : AbstractScraper
    {
        ...
        public DanmuApi(ILogManager logManager, IJsonSerializer jsonSerializer, IHttpClient httpClient)
            : base(logManager.getDefaultLogger("DanmuApi"))
        {
            _api = new DanmuApiApi(logManager, httpClient);
        }
        ...
        public override async Task<string> SearchMediaId(BaseItem item)
        {
            ...
            foreach (var anime in animes)
            {
                ...
                {
                    log.Info("[{0}] 发行年份不一致，忽略处理. 年份：{1} emby: {2}", title, anime.Year, itemPubYear);
                    continue;
                }
