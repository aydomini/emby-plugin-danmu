            ...
            try
            {
                ...
                if (id.StartsWith("BV", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (videoInfo != null)
                    {
                        ...
                        // 优先检查是否为合集
                        if (videoInfo.UgcSeason != null && videoInfo.UgcSeason.Sections != null && videoInfo.UgcSeason.Sections.Any())
                        {
                            log.Info($"Bilibili.GetMedia (BVID: {id}): 检测到合集，共 {videoInfo.UgcSeason.Sections.Count} 个分区。");
                            // 使用合集的第一个分区的剧集
                            var section = videoInfo.UgcSeason.Sections[0];
                            if (section.Episodes != null && section.Episodes.Any())
                            {
                                foreach (var ugcEp in section.Episodes)
                                {
                                    log.Debug($"GetMedia (BVID 合集: {id}): 添加合集剧集。CID: {ugcEp.CId}, 标题: {ugcEp.Title}");
                                ...
                                log.Info($"Bilibili.GetMedia (BVID 合集: {id}): 从合集中添加了 {scraperMedia.Episodes.Count} 个剧集。");
                            }
                            else
                            {
                                log.Info($"Bilibili.GetMedia (BVID 合集: {id}): 合集分区中没有剧集。");
                            }
                        }
                        // 否则使用分P视频
                        else if (videoInfo.Pages != null && videoInfo.Pages.Any())
                        {
                            log.Info($"Bilibili.GetMedia (BVID: {id}): 检测到分P视频，共 {videoInfo.Pages.Count} 个分P。");
                        ...
                        {
                            log.Info($"Bilibili.GetMedia (BVID: {id}): 找到了 '{item.Name}' 的视频信息，但既不是合集也没有列出分P。");
                        }

                        // 如果是电影类型，设置主 CommentId
                        if (isMovieItemType && scraperMedia.Episodes.Any())
                        {
                            // 对于电影，将主 CommentId 设置为第一个分P/剧集的 CID
                            var firstEpisodeCommentId = scraperMedia.Episodes.First().CommentId;
                            scraperMedia.CommentId = firstEpisodeCommentId;
                            log.Info($"Bilibili.GetMedia (BVID: {id}): Emby 项目 '{item.Name}' 是电影。将 ScraperMedia.CommentId 设置为第一个剧集的 CID: '{firstEpisodeCommentId}'");
                        }
                    }
